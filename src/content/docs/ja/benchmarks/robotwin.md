---
title: RoboTwin評価
description: RoboTwin 2.0ベンチマークでのStarVLA実験結果の再現。
---

**RoboTwin 2.0**は、50タスク、2つの難易度レベル（Easy / Hard、シーンランダム化の違い）を持つ双腕ロボット操作ベンチマークで、把持、積み重ね、道具使用、その他さまざまな操作タイプをカバーしています。

このドキュメントでは、[RoboTwin 2.0](https://github.com/RoboTwin-Platform/RoboTwin)での**実験結果**を再現するための手順を提供します。

評価プロセスは以下の2つの主要な部分で構成されています：

1. `robotwin` 環境と依存関係のセットアップ。
2. `starVLA` と `robotwin` の両方の環境でサービスを起動して評価を実行。

:::note[2つのターミナルを使う理由]
モデル推論（starVLA環境）とシミュレーション（robotwin環境）は、同じconda環境にインストールすると競合する異なるPythonパッケージバージョンに依存しています。別々のconda環境で別々のターミナルで実行することで、この問題を回避できます。
:::
...
環境をセットアップするには、まず[公式RoboTwinインストールガイド](https://robotwin-platform.github.io/doc/usage/robotwin-install.html)に従って、ベースの `robotwin` 環境をインストールしてください。
...
`examples/Robotwin/eval_files/eval.sh` の `ROBOTWIN_PATH` を編集してください。
...
`examples/Robotwin/eval_files/deploy_policy.yml` と `examples/Robotwin/eval_files/run_policy_server.sh` のチェックポイントパスを編集してください。

---

#### ステップ2. シミュレーションの起動（robotwin環境）

2番目のターミナルで `robotwin` conda環境をアクティベートして実行します：
...
すべてのモードには `demo_clean` と `demo_randomized` が含まれます。
