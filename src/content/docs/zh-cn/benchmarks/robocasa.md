...
:::note[为什么需要两个终端？]
模型推理（starVLA 环境）和仿真环境（robocasa 环境）各自依赖不同的 Python 包版本，放在同一个 conda 环境中会产生冲突。因此需要分别在两个终端中激活各自的 conda 环境来运行。
:::
...
| 任务 | GR00T-N1.6 | StarVLA-GR00T-Qwen3 | StarVLA-π-Qwen3 | StarVLA-OFT-Qwen3 | StarVLA-FAST-Qwen3 |
|------|------------|------------|---------|----------|-----------|
